      ...
      <div className="mx-auto max-w-5xl px-5 py-28 sm:py-36">
        ...
        <Reveal className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-8 text-lg leading-relaxed text-foreground/80 text-pretty">
            Значительную часть основной техники и производственной базы предоставляет ВГИК. Но чтобы
            провести съёмки, часть производства нам необходимо оплатить самостоятельно.
          </p>
        </Reveal>

        {/* Goal + achievability */}
        <Reveal delay={100} className="mx-auto mt-16 max-w-2xl text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.3em] text-thread">Цель сбора</p>
        ...
        <div className="mt-24">
          <Reveal className="mb-10 text-center">
            <h3 className="font-serif text-2xl text-ink sm:text-3xl">На что пойдут средства</h3>
          </Reveal>
          <div className="grid gap-px overflow-hidden rounded-sm border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
            {MONEY_USAGE.map((item, i) => (
              <Reveal key={item.title} delay={(i % 3) * 70} className="bg-card">
                <div className="h-full px-6 py-7">
                  <span className="inline-block size-2 rotate-45 bg-thread" aria-hidden="true" />
        ...
        <Reveal className="mt-20">
          <div className="flex flex-col items-center gap-6 rounded-sm border border-gold/50 bg-gold/10 px-6 py-12 text-center sm:px-12">
            ...
            <SupportButton amount={5000}>Поддержать на 5 000 ₽</SupportButton>
          </div>
        </Reveal>
      </div>
    </section>
