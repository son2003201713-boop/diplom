...
export function DonationModal() {
  ...
  const [selected, setSelected] = useState<number | null>(FUNDRAISING.amounts[1])
  ...
  const allRequisites = bankRows.map((r) => `${r.label}: ${r.value}`).join("\n")
    ...
    >
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 px-6 pt-5 pb-4">
          <div>
            <h2 id="donation-title" className="font-serif text-2xl leading-tight text-ink">
              Поддержать «Три солнца»
            </h2>
            <p className="mt-1.5 text-sm leading-relaxed text-muted-foreground text-pretty">
              Выберите сумму. Даже небольшая поддержка помогает съёмкам.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="overflow-y-auto px-6 pb-6">
          ...
          <div className="mt-2.5">
            ...
            >
              <input
                ...
                className="w-full bg-transparent text-sm font-semibold uppercase tracking-[0.08em] text-ink outline-none placeholder:text-muted-foreground placeholder:tracking-[0.12em]"
              />
              {custom && <span className="text-sm font-semibold text-ink">₽</span>}
            </div>
          </div>

          {activeAmount != null && activeAmount >= FUNDRAISING.creditThreshold && (
            <p className="mt-3 rounded-md border border-gold/50 bg-gold/10 px-4 py-3 text-xs leading-relaxed text-ink">
              Спасибо! Поддержка от {formatAmount(FUNDRAISING.creditThreshold)} ₽ — мы укажем ваше
          ...
          <div className="mt-4 overflow-hidden rounded-md border border-border">
            {/* QR donation */}
<div className="mt-5 rounded-md border border-border bg-card px-4 py-5">
  <div className="flex flex-col items-center text-center">
    ...
    <p className="mt-2 max-w-xs text-sm leading-relaxed text-muted-foreground">
      Отсканируйте QR-код камерой телефона, чтобы перейти к переводу.
    </p>
    ...
    <p className="mt-3 text-xs text-muted-foreground">
      Если QR-код не подходит, ниже можно перевести по реквизитам.
    </p>
  </div>
</div>
            <button
              ...
              <div className="animate-fade-in border-t border-border bg-linen px-4 py-4">
                <dl className="divide-y divide-border">
                  {bankRows.map((row) => (
                    <div key={row.label} className="flex flex-col gap-1 py-2.5">
                      ...
                      <dd className="text-sm leading-snug text-ink break-words">{row.value}</dd>
                    </div>
                  ))}
                </dl>

                <p className="mt-3 text-xs leading-relaxed text-muted-foreground text-pretty">
                  При переводе по реквизитам, пожалуйста, укажите назначение платежа без изменений.
                </p>
                ...
                <div className="mt-4 flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <CopyButton value={BANK.purpose} label="Скопировать назначение" />
                    <CopyButton value={allRequisites} label="Скопировать все реквизиты" />
                  </div>
                </div>
              </div>
            )}
  ...
  <p className="text-xs leading-relaxed text-muted-foreground">
    ...
    некоторые браузеры могут показывать предупреждение безопасности
    при переходе к российским сервисам. В таком случае попробуйте
    открыть оплату в Яндекс Браузере или выберите перевод по QR-коду.
  </p>
</div>
          <p className="mt-4 text-center text-xs leading-relaxed text-muted-foreground">
            Регистрация не требуется. Каждый перевод приближает один из наших съёмочных дней.
          </p>
        </div>
      </div>
    </div>
